...
const getListMessageOfGroup = async (req, res, next = function(err) {
    ...
    try {
        ...
        // let page= parseInt(req.query.page);
        // let limit = parseInt(req.query.limit);
        const existGroup = groupRepository.getOne({where: { _id: data.group, members: author}, select: '_id'});
        if (!existGroup) {
            return next(new Error('NOT_EXISTED_GROUP'));
        }
        const messages = await messageRepository.getAll({ where : { group : data.group }, select: 'content author createdAt', populate: {
            path: 'author',
            select: 'username'
        }, sort: {'createAt': -1}});
        // return res.status(200).json({
